...
@RestController // return karana data eka automatically HTTP response body ekata serialize
				// wenawa (mostly JSON format eka).

public class EmployeeController {

	@Autowired // variable/constructor/setter ekata gelapana Bean ekak container eka thiye nam,
				// eka auto-assign (inject) karanawa.
	...
	// public EmployeeController(EmployeeDao employeeDao, EmployeeStatusDao
	// employeeStatusDao) {
	...
	// ema data object eka mema method ekedi puluwan RequestBody yana anotation eken
	// access karanna
	...
	public String saveEmployeeData(@RequestBody Employee employee) {
		...
		try {
			...
			// Dao gawa thibena save operator ekakata api gawa thibena employee entitiya
			// pass kirima
			employeeDao.save(employee);

			// dependances
			// employee account ekak hedena kota ema employeeta user account ekak onnam eya
			// methanin hediya yuthuya
			// user account ekak create kirimata kalin ema userta account ekak awashyada
			// belima
			if (employee.getDesignation_id().getUseraccount()) {
				...
				// employee object ekak hedenakota create wena user account ekata adala employee
				// kawda yana bawa add wima
...
//        employee delete nm
		...
		// existing check kalata pasuwa eya exist wenawa nam awlak ne deleta karanna ena
		// object eka thiyenawa ema object eka update karanna gatha heka deleta status
		// change we
		...
		try {
			...
			// me sadaha employee status type object ekak illai e sadaha employeeStausDao
			// autowired kara gatha yuthuya
			extEmp.setEmployeestatus_id(employeeStatusDao.getReferenceById(3));

			// pass wena employee object eken exist employee record ekak id eken aran save
			// karai eyata hethuwa pass wena object ekehi data wenas wela awoth ewath save
			// wana nisa

			// update operation ekath save magin sidu wei
			// delete,update ekata ewana object eka PK ekak atha ema nisa PK walin save
			// waladi update ekak sidu wei
			// namuth post ekata ewana object eka PK netha fontend ekan ewana empty object
			// eka atha
			// delete ekata fontend eken ena object eka data wenas wimak sidu wee save wima
			// walakwimata ema record eka save nokara ehi id eken DB eke thibena record eka
			// gena save karai
	...
	public String updateEmployeeData(@RequestBody Employee employee) {
		...
		// check ext - pitin ena data ekata id ekaka add kala nohaka eaya db eka ai
		// magin lebe.
		...
		// duplicate check
		// mehidee update kala value eka duplicatda belima saha ema field eka update
		// nokala awasthawaka samana value ekata edite karana employeewa athulath
		// nowimata id asamanada beliya yuthuya
	...
	// load employee whithout employee user account data
